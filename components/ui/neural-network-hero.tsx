...
// We implement a simplified version or use standard GSAP if it's not available in the environment.
// In a real production setup, you'd import it from 'gsap/SplitText'.
// For this implementation, we use a robust text animation approach.
...
// Internal catalogs for R3F elements that might be missing from JSX.IntrinsicElements 
// due to environment configuration or global namespace conflicts.
const Mesh = 'mesh' as any;
const PlaneGeometry = 'planeGeometry' as any;
const CPPNShaderMaterialElement = 'cPPNShaderMaterial' as any;
...
function ShaderPlane() {
  ...
  return (
    <Mesh ref={meshRef} position={[0, -0.75, -0.5]}>
      ...
      <CPPNShaderMaterialElement ref={materialRef} side={THREE.DoubleSide} />
    </Mesh>
  );
}
...
export default function Hero({
  ...
  useGSAP(
    () => {
      ...
      // Animation logic - simplified to not depend on SplitText plugin specifically
      ...
      if (badgeRef.current) {
        tl.set(badgeRef.current, { autoAlpha: 0, y: -8 });
      ...
      if (paraRef.current) {
        tl.set(paraRef.current, { autoAlpha: 0, y: 8 });
        tl.to(paraRef.current, { autoAlpha: 1, y: 0, duration: 0.6 }, '-=0.6');
      }
      
      if (ctaRef.current) {
        tl.set(ctaRef.current, { autoAlpha: 0, y: 8 });
      ...
      if (microItems.length > 0) {
        tl.set(microItems, { autoAlpha: 0, y: 6 });
        tl.to(microItems, { autoAlpha: 1, y: 0, duration: 0.6, stagger: 0.1 }, '-=0.3');
      }
    },
    { scope: sectionRef },
  );
